...
class StudentInterestsView(UpdateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, 'ინტერესები წარმატებულად განახლდა')
...
def take_quiz(request, pk):
    ...
    if student.quizzes.filter(pk=pk).exists():
        return redirect('students:taken_quiz_list.html', pk)
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                if student.get_unanswered_questions(quiz).exists():
                    ...
                    student.save()
                    if percentage < 50.0:
                        messages.warning(request, 'წარმატებები შემდეგ ჯერზე, შენი შეფასება ქვიზისთვის: %s არის %s.' % (
                            quiz.name, percentage))
                    else:
                        messages.success(request,
                                         'გილოცავ! შენ შეასრულე ქვიზი: %s წარმატებით! შენი შეფასებაა %s ქულა.' % (
                                             quiz.name, percentage))
                    # return redirect('students:quiz_list')

...
class QuizCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        messages.success(self.request, 'ქვიზი წარმატებიტ შეიქმნა, ახლა შეგიძლიათ შეკითხვების დამატებაც')
...
class QuizDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(request, 'ქვიზი: %s წარმატებით წაიშალა' % quiz.name)
...
def question_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question.quiz = quiz
            question.save()
            messages.success(request, 'შენ შეიძლება არ დაამატე შეკითხვა/პასუხი ქვიზში')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                form.save()
                formset.save()
            messages.success(request, 'შეკითხვა და პასუხები წარმატებით იქნა შენახული')
            return redirect('teachers:quiz_change', quiz.pk)
    else:
...
class QuestionDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        question = self.get_object()
        messages.success(request, 'შეკითხვა %s წარმატებულად იქნა წაშლილი!' % question.text)
        return super().delete(request, *args, **kwargs)
